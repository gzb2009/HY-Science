...
import { getDirectory as _getDirectory, getFilename } from "@hysci/util/path"
...
export function UserMessageDisplay(props: { message: UserMessage; parts: PartType[]; onRevert?: () => void }) {
  ...
  const textPart = createMemo(
    () =>
      props.parts?.find(
        (p) => p.type === "text" && !(p as TextPart).hybio && !(p as TextPart & { synthetic?: boolean }).synthetic,
  ...
  const attachments = createMemo(() =>
    files()?.filter((f) => {
      // Inline @-mentions stay in the text highlight path; everything else
      // (drag-drop CSV/PDF/images, etc.) renders as an attachment chip.
  ...
  const openImagePreview = (url: string, alt?: string) => {
    dialog.show(() => <ImagePreview src={url} alt={alt} />, { lite: true })
  }
  ...
  return (
    <div data-component="tool-part-wrapper" data-permission={showPermission()} data-question={showQuestion()}>
      ...
      <Show when={showPermission() && permission()}>
        <PermissionActions respond={respond} />
      </Show>
...
function PermissionActions(props: { respond: (response: "once" | "always" | "reject") => void }): JSX.Element {
  const i18n = useI18n()
  return (
...
function createOutputToolRender(config: {
  icon: string
  titleKey: string
  subtitle: (input: Record<string, any>) => string
  args?: (input: Record<string, any>) => string[]
  action?: () => JSX.Element
}) {
  return function OutputTool(props: any) {
    const i18n = useI18n()
    return (
      <BasicTool
        {...props}
        icon={config.icon}
        trigger={{
          title: i18n.t(config.titleKey as any),
          subtitle: config.subtitle(props.input),
          args: config.args?.(props.input) ?? [],
...
ToolRegistry.register({
  name: "list",
  render: createOutputToolRender({
    icon: "bullet-list",
    titleKey: "ui.tool.list",
    subtitle: (i) => getDirectory(i.path || "/"),
  }),
})
...
ToolRegistry.register({
  ...
  render: createOutputToolRender({
    icon: "magnifying-glass-menu",
    titleKey: "ui.tool.glob",
    subtitle: (i) => getDirectory(i.path || "/"),
    args: (i) => (i.pattern ? ["pattern=" + i.pattern] : []),
  }),
})
...
ToolRegistry.register({
  ...
  render: createOutputToolRender({
    icon: "magnifying-glass-menu",
    titleKey: "ui.tool.grep",
    subtitle: (i) => getDirectory(i.path || "/"),
    args: (i) => {
      const a: string[] = []
      if (i.pattern) a.push("pattern=" + i.pattern)
      if (i.include) a.push("include=" + i.include)
      return a
    },
  }),
})
...
ToolRegistry.register({
  ...
  render: createOutputToolRender({
    icon: "window-cursor",
    titleKey: "ui.tool.webfetch",
    subtitle: (i) => i.url || "",
    args: (i) => (i.format ? ["format=" + i.format] : []),
    action: () => (
...
function QuestionPrompt(props: { request: QuestionRequest }) {
  const data = useData()
  const i18n = useI18n()
  const decision = createMemo(() => props.request.decision)
  const recommended = createMemo(() => props.request.auto === "recommended")
  const [remaining, setRemaining] = createSignal(0)
  const [counting, setCounting] = createSignal(true)
  ...
  function defaultAnswer(q: QuestionInfo): string {
    ...
    const marked = q.options.find((opt) => /\(Recommended\)|（推荐）|推荐/.test(opt.label))
    if (marked) return marked.label
    return q.options[0]?.label || i18n.t("ui.question.skip.default")
  }
