...
export namespace Config {
  ...
  function getManagedConfigDir(): string {
    const dir = (() => {
      switch (process.platform) {
        case "darwin":
          return "/Library/Application Support/hyscience"
        case "win32":
          return path.join(process.env.ProgramData || "C:\\ProgramData", "hyscience")
        default:
          return "/etc/hyscience"
      }
    })()
    // Enterprise machines provisioned before the HYscience rename may still
    // use the legacy "synsc" directory; keep honoring it until re-provisioned.
    if (existsSync(dir)) return dir
    const old = dir.replace(/hyscience$/, "synsc")
    return existsSync(old) ? old : dir
  }

  const managedConfigDir = process.env.HYSCIENCE_TEST_MANAGED_CONFIG_DIR || getManagedConfigDir()

  // Config filenames, oldest first: later merges win, so the legacy names load
  // as the base and hyscience.json(c) overrides them.
  const CONFIG_FILES = ["synsc.jsonc", "synsc.json", "hyscience.jsonc", "hyscience.json"]
  ...
  export const state = Instance.state(async () => {
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        log.debug("fetching remote config", { url: `${key}/.well-known/hyscience` })
        const response = await fetch(`${key}/.well-known/hyscience`)
        if (!response.ok) {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://hyscience.ai/config.json"
        result = mergeConfigConcatArrays(
          result,
          await load(JSON.stringify(remoteConfig), `${key}/.well-known/hyscience`),
        )
    ...
    if (Flag.HYSCIENCE_CONFIG) {
      result = mergeConfigConcatArrays(result, await loadFile(Flag.HYSCIENCE_CONFIG))
      log.debug("loaded custom config", { path: Flag.HYSCIENCE_CONFIG })
    }

    // Project config has highest precedence (overrides global and remote)
    if (!Flag.HYSCIENCE_DISABLE_PROJECT_CONFIG) {
    ...
    if (Flag.HYSCIENCE_CONFIG_CONTENT) {
      result = mergeConfigConcatArrays(result, JSON.parse(Flag.HYSCIENCE_CONFIG_CONTENT))
      log.debug("loaded custom config from HYSCIENCE_CONFIG_CONTENT")
    }
    ...
    const directories = [
      Global.Path.config,
      // Only scan project .hyscience/ directories when project discovery is enabled
      // (".synsc" is the pre-rename name, still honored)
      ...(!Flag.HYSCIENCE_DISABLE_PROJECT_CONFIG
        ? await Array.fromAsync(
            Filesystem.up({
              targets: [".hyscience", ".synsc"],
              start: Instance.directory,
              stop: Instance.worktree,
            }),
          )
        : []),
      // Always scan ~/.hyscience/ (user home directory)
      ...(await Array.fromAsync(
        Filesystem.up({
          targets: [".hyscience", ".synsc"],
    ...
    if (Flag.HYSCIENCE_CONFIG_DIR) {
      directories.push(Flag.HYSCIENCE_CONFIG_DIR)
      log.debug("loading config from HYSCIENCE_CONFIG_DIR", { path: Flag.HYSCIENCE_CONFIG_DIR })
    }

    for (const dir of unique(directories)) {
      if (dir.endsWith(".hyscience") || dir.endsWith(".synsc") || dir === Flag.HYSCIENCE_CONFIG_DIR) {
        for (const file of CONFIG_FILES) {
    ...
    // Written by HYscience.syncServices() to the user's XDG config dir. Tolerate
    ...
    const syncedConfig = path.join(Global.Path.config, "hyscience-synced.json")
    try {
      result = mergeConfigConcatArrays(result, await loadFile(syncedConfig))
    } catch (error) {
      log.error("failed to load synced config; ignoring file until it is repaired", {
        path: syncedConfig,
    ...
    if (Flag.HYSCIENCE_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.HYSCIENCE_PERMISSION))
    }
    ...
    if (Flag.HYSCIENCE_DISABLE_AUTOCOMPACT) {
      result.compaction = { ...result.compaction, auto: false }
    }
    if (Flag.HYSCIENCE_DISABLE_PRUNE) {
  ...
  export async function installDependencies(dir: string) {
    ...
    await BunProc.run(
      ["add", "@hysci/plugin@" + (Installation.isLocal() ? "latest" : Installation.VERSION), "--exact"],
      {
        cwd: dir,
      },
    ).catch(() => {})
   ...
   * getPluginName("oh-my-hyscience@2.4.3") // "oh-my-hyscience"
   ...
   * 2. Local hyscience.json
   * 3. Global plugin/ directory
   * 4. Global hyscience.json
   *
  ...
  export function deduplicatePlugins(plugins: string[]): string[] {
    ...
    // e.g., "oh-my-hyscience", "@scope/pkg"
    const seenNames = new Set<string>()

    // uniqueSpecifiers: full plugin specifiers to return
    // e.g., "oh-my-hyscience@2.4.3", "file:///path/to/plugin.js"
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://hyscience.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://hyscience.ai/docs/agents"),
      provider: z
      ...
      compaction: z
        .object({
          ...
          protectedTools: z.array(z.string()).optional().describe("Additional tool IDs to protect from pruning"),
        })
        .optional(),
      experimental: z
        .object({
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "hyscience.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "hyscience.jsonc"))),
    )
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://hyscience.ai/config.json"
          result = mergeDeep(result, rest)
          await Bun.write(path.join(Global.Path.config, "config.json"), JSON.stringify(result, null, 2))
          await fs.unlink(legacy)
        })
        .catch((error) => {
          log.error("failed to migrate legacy config; leaving original file in place", {
            path: legacy,
            error: error instanceof Error ? error.message : String(error),
          })
        })
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://hyscience.ai/config.json"
        // Write the $schema to the original text to preserve variables like {env:VAR}
        const updated = original.replace(/^\s*\{/, '{\n  "$schema": "https://hyscience.ai/config.json",')
      ...
      if (data.plugin) {
        for (let i = 0; i < data.plugin.length; i++) {
          ...
          try {
            ...
            log.error("failed to resolve plugin", {
              plugin,
              path: configFilepath,
              error: message,
            })
            throw new InvalidError(
              {
                path: configFilepath,
                message: `Failed to resolve plugin "${plugin}": ${message}`,
              },
              { cause: error },
            )
          }
  ...
  function globalConfigFile() {
    const candidates = ["hyscience.jsonc", "hyscience.json", "config.json"].map((file) =>
  ...
  function projectConfigFile() {
    const candidates = [
      path.join(Instance.worktree, "hyscience.jsonc"),
      path.join(Instance.worktree, "hyscience.json"),
      path.join(Instance.worktree, ".hyscience", "hyscience.jsonc"),
      path.join(Instance.worktree, ".hyscience", "hyscience.json"),
