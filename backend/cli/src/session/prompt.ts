...
export namespace SessionPrompt {
  ...
  export const OUTPUT_TOKEN_MAX = Flag.HYSCIENCE_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000

  export function atStepLimit(step: number, maxSteps: number) {
    return step >= maxSteps
  }

  export function toolsAtStep<T>(tools: T, atLimit: boolean) {
    if (atLimit) return {}
    return tools
  }
  ...
  export const PromptInput = z.object({
    ...
    mergeTaskContext: z.boolean().optional(),
    parts: z.array(
  ...
  export const prompt = fn(PromptInput, async (input) => {
    ...
    const isFirstScope = !session.taskScope
    const taskScope = TaskScope.resolve(session.taskScope, messages, message, input.newTask, input.mergeTaskContext)
    if (taskScope !== session.taskScope) {
      session.taskScope = taskScope
      await Session.update(session.id, (draft) => {
        draft.taskScope = taskScope
      })
    }
    const task = TaskScope.current(taskScope)
    if (isFirstScope && task) await ResearchContext.migrate(input.sessionID, task.id)
  ...
  export function cancel(sessionID: string) {
    ...
    // Always clear pending questions, even when the prompt loop entry is gone —
    // otherwise a stuck question card can leave the UI unable to stop/skip.
    void Question.cancelSession(sessionID)
    const s = state()
    ...
    while (true) {
      ...
      if (!model) {
        const error = new NamedError.Unknown({
          message: `Model ${lastUser.model.providerID}/${lastUser.model.modelID} is not available. Add your own API key (\`hyscience keys add\`) or connect a managed account (\`hyscience login\`), then choose a model.`,
        }).toObject()
      ...
      const cb = CircuitBreaker.update(breaker, msgs, step)
      if (cb.warning) {
        const cbPart: MessageV2.TextPart = {
          id: Identifier.ascending("part"),
          messageID: "",
          sessionID,
          type: "text",
        ...
        if (lastUserMsg) {
          cbPart.messageID = lastUserMsg.info.id
          lastUserMsg.parts.push(cbPart as any)
        }
      }
      ...
      if (step > 1 && lastFinished) {
        for (const msg of sessionMessages) {
          ...
          for (const part of msg.parts) {
            if (part.type !== "text" || part.ignored || MessageV2.isHybio(part)) continue
  ...
  async function createUserMessage(input: PromptInput) {
    ...
    const requestedAgent = await Agent.get(requested)
    const routed =
      requestedAgent.mode === "primary" || requestedAgent.mode === "all"
  ...
  async function ensureTitle(input: {
    ...
    // Find first non-hybio user message
    const firstRealUserIdx = input.history.findIndex(
      (m) => m.info.role === "user" && !m.parts.every((p) => MessageV2.isHybio(p as any)),
    )
    if (firstRealUserIdx === -1) return

    const isFirst =
      input.history.filter((m) => m.info.role === "user" && !m.parts.every((p) => MessageV2.isHybio(p as any)))
        .length === 1
    if (!isFirst) return
